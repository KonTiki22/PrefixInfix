...
fun checkAnswer(answer:String): Boolean {
    if(answer == "") {
        println("Некорректный ввод. Пустая строка.")
        return false
    }
    if(Regex("""([^0-9/*+. -])""").containsMatchIn(answer)) {
        println("Некорректный ввод. Выражение содержит недопустимые символы.")
        return false
    }
    ...
    if(count_operators+1 != count_operands) {
        println("Некорректный ввод. Выражение содержит неправильное соотношение операторов и операндов.")
        return false
    }
    return true
}
...
fun main(){
    while(true) {
        print("\nВведите выражение в префиксной нотации.\n" +
                "Выражение может содержать только бинарные арифметические операторы и числа, разделенные пробелом.\n" +
                "Для запуска теста введите test.\n\nВвод: ")
        val answer: String = readLine().toString().trim()
        ...
        if(answer == "test") {
            ...
            if(checkAnswer(answer)) {
                println("Результат: ${transformExpression(answer)}")
                break
            }
        }
    }
